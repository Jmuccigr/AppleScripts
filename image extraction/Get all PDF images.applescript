...
on open fList
	repeat with fname in (fList)
		...
		if ext is not "pdf" then
			display alert "Wrong file type" message "This does not appear to be a PDF file. Skipping."
			exit repeat
		end if
		
		# Set file types to be handled
		set filetypes to "  -png -tiff -j -jp2 -ccitt "
		set filetypes to "  -all "
		...
		tell application "Finder"
			set fnameString to characters 1 thru 35 of (((name of file fname) as string) & "              ") as string
		...
		# Add a little delay to make sure the folders have different names
		delay 1
	end repeat
