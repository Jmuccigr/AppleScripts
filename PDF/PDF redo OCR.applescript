...
on open of finderObjects
	...
	try
		# Get starting-page number and OCR language(s)
		...
		# Coy the file to the tmpdir, removing the irrelevant pages if needed, to save a little time in OCR
		try
		...
		on error errMsg number errNum
			if errMsg is not "User canceled." then display alert "Problem" message "There was a problem creating the temporary PDF: " & return & errMsg
			error number -128
		end try
		
		# Do the OCR
		display notification ("Now doing OCR...") with title "OCR" sound name "beep"
		set theResult to (do shell script (ocrPath & " -l " & lang & " --redo-ocr " & pfile & space & tmpdir & dateString & ".pdf"))
		...
		on error errMsg number errNum
			if errMsg is not "User canceled." then display alert "Problem" message "There was a problem creating the final PDF: " & return & errMsg
			error number -128
		end try
	on error errMsg number errNum
