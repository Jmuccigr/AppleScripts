...
on open of finderObjects
	...
	repeat with filename in (finderObjects)
		...
		do shell script "size=`$(bash -l -c 'which magick') " & fname & " -format " & quote & "%[fx:" & pct & "*min(w,h)/100]" & quote & " info:`\n$(bash -l -c 'which magick') " & fname & " +repage \\( -size ${size}x${size} -background white xc: \\) -gravity northwest -compose over -composite -fuzz 2% -trim -bordercolor white -border " & border & " +repage $TMPDIR/tempfile.png"
