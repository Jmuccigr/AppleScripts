-- Trim left side of images using imagemagick
...
on open of finderObjects
	...
	repeat with filename in (finderObjects)
		...
		do shell script "lw=`$(bash -l -c 'which magick') " & fname & " -format " & quote & "%[fx:max(5,ceil(w*" & pct & "/100))]\" info:`; \n\t\t$(bash -l -c 'which magick') " & fname & " +repage -gravity west -chop ${lw}x0 -fuzz 2% -trim -bordercolor white -border " & border & " +repage $TMPDIR/tempfile.png"
