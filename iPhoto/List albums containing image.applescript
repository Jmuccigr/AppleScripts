...
tell application "iPhoto"
	...
	try
		...
		repeat with i from 1 to photoCount
			...
			set album_IDs to (the id of every album whose id of the photos contains this_photo_id and type is regular album)
...
on getalbumpath(albumIDs)
	tell application "iPhoto"
		set theNames to {}
		repeat with albumID in albumIDs
			set calbum to album id albumID
			set theName to the name of calbum
			repeat until not (the parent of the calbum exists)
				set calbum to the parent of calbum
				set theName to the name of calbum & " > " & theName
			end repeat
